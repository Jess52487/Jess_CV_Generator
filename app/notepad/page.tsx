...
export default function Notepad() {
  ...
  const saveExpForm = () => {
    if (currentExp) {
      if (data.experiences.some(e => e.id === currentExp.id)) updateExperience(currentExp.id, currentExp);
      else addExperience(currentExp);
    }
  ...
  const openNewEduForm = () => {
    setCurrentEdu({ id: Date.now().toString(), institution: "", degree: "", startYear: "", endYear: "" });
  ...
  const saveEduForm = () => {
    if (currentEdu) {
      if (data.education.some(e => e.id === currentEdu.id)) updateEducation(currentEdu.id, currentEdu);
      else addEducation(currentEdu);
    }
                    ...
                    {isEditingExp && currentExp && (
                      <div className="mb-8 p-6 bg-yellow-50 border border-yellow-200 shadow-lg rounded-sm relative transform rotate-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                          ...
                          <div className="space-y-1"><label className="text-xs uppercase font-[family-name:var(--font-label-stamp)] text-yellow-800">Start Date</label><input type="text" className="w-full bg-white/50 border-b border-yellow-300 px-2 py-1 outline-none font-[family-name:var(--font-body-md)]" value={currentExp.startDate} onChange={e => setCurrentExp({...currentExp, startDate: e.target.value})} placeholder="e.g. 2018"/></div>
                          <div className="space-y-1"><label className="text-xs uppercase font-[family-name:var(--font-label-stamp)] text-yellow-800">End Date</label><input type="text" className="w-full bg-white/50 border-b border-yellow-300 px-2 py-1 outline-none font-[family-name:var(--font-body-md)]" value={currentExp.endDate} onChange={e => setCurrentExp({...currentExp, endDate: e.target.value})} placeholder="e.g. Present"/></div>
                    ...
                    {isEditingEdu && currentEdu && (
                      <div className="mb-8 p-6 bg-blue-50 border border-blue-200 shadow-lg rounded-sm relative transform -rotate-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                          ...
                          <div className="space-y-1"><label className="text-xs uppercase font-[family-name:var(--font-label-stamp)] text-blue-800">End Year</label><input type="text" className="w-full bg-white/50 border-b border-blue-300 px-2 py-1 outline-none font-[family-name:var(--font-body-md)]" value={currentEdu.endYear} onChange={e => setCurrentEdu({...currentEdu, endYear: e.target.value})} placeholder="e.g. 2016"/></div>
                        </div>
                        <div className="flex justify-end gap-4 mt-6">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {(data.education || []).map((edu) => (
                        <div key={edu.id} className="relative group bg-white border border-[var(--color-outline-variant)] p-4 rounded-sm shadow-sm flex flex-col md:flex-row justify-between md:items-center">
                          ...
                          <div>
                            <h3 className="font-[family-name:var(--font-body-lg)] text-[18px] font-bold text-[var(--color-on-surface)] pr-16">{edu.institution}</h3>
                            <p className="font-[family-name:var(--font-body-md)] text-[16px] text-[var(--color-primary)]">{edu.degree}</p>
                          </div>
                          <div className="mt-2 md:mt-0 font-[family-name:var(--font-label-embossed)] text-[12px] text-[var(--color-on-surface-variant)]">
